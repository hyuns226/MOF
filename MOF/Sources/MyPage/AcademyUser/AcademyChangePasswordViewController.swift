//
//  AcademyChangePasswordViewController.swift
//  MOF
//
...
import Foundation
import UIKit
class AcademyChangePasswordViewController : UIViewController{
    
    let dataManager = AcademyMyPageDataManager()
    ...
    @IBAction func changePasswordButtonAction(_ sender: Any) {
        ...
        dataManager.password(passwordInput, academyIdx: KeyCenter.userIndex, delegate: self)
...
//MARK:- API
extension AcademyChangePasswordViewController {
    func password(result : passwordResponse){
        if result.isSuccess{
